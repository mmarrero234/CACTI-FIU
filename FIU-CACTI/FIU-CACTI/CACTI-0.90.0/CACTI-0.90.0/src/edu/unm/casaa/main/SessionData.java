...
public class SessionData {
	...
	private boolean isSQLiteDataFile() throws IOException {
		...
		// Test selected file format by looking for "SQLite" at beginning of
		// file
		try (FileReader textFileReader = new FileReader(sessionFile)) {
			char[] buffer = new char[6];
			int numberOfCharsRead = textFileReader.read(buffer);
			// System.out.println("XX" + String.valueOf(buffer, 0,
			// numberOfCharsRead) + "XX");
	...
	public ArrayList<Integer> getUtteranceRatingIDs(String utterance_id) throws SQLException {
		...
		try (Connection connection = ds.getConnection();
				PreparedStatement ps = connection.prepareStatement(
						"select ratings.rating_id from ratings left outer join utterances_ratings on utterances_ratings.rating_id = ratings.rating_id where utterances_ratings.utterance_id = ?;")) {
	...
	public ArrayList<TreeMap<String, String>> getGlobalUtterances() throws SQLException {
		String sql = "SELECT ratings.rating_id, ratings.rating_name, ratings.response_value, utterances.utterance_id, utterances.time_marker, utterances.annotation, codes.code_name "
				+ "FROM ratings " + "LEFT JOIN utterances_ratings ON ratings.rating_id = utterances_ratings.rating_id "
	...
	private SortedMap<String, Utterance> getUtterances() throws SQLException {
		...
		try (Connection connection = ds.getConnection(); Statement statement = connection.createStatement()) {
			...
			ResultSet rs = statement.executeQuery(
					"select utterances.*, codes.code_name, codes.speaker_id from utterances inner join codes on utterances.code_id = codes.code_id order by utterances.time_marker");
	...
	private HashMap<String, Integer> getCodeCounts() throws SQLException {
		...
		try (Connection connection = ds.getConnection(); Statement statement = connection.createStatement()) {
			...
			ResultSet rs = statement.executeQuery(
					"select codes.code_name, count(codes.code_id) as code_count from codes join utterances on codes.code_id = utterances.code_id group by codes.code_name;");
	...
	public HashMap<String, Double> getCodeSummaryMap() throws SQLException {
		...
		mapCodeSummary.put("SUM_CHANGE_ETOH",
				mapCodeCount.entrySet().stream()
						.filter(e -> e.getKey().equals("C+") || e.getKey().equals("R+") || e.getKey().equals("D+")
								|| e.getKey().equals("A+") || e.getKey().equals("N+") || e.getKey().equals("TS+")
								|| e.getKey().equals("O+"))
		...
		mapCodeSummary.put("SUM_CHANGE_DRUG",
				mapCodeCount.entrySet().stream()
						.filter(e -> e.getKey().equals("C+_m") || e.getKey().equals("R+_m") || e.getKey().equals("D+_m")
								|| e.getKey().equals("A+_m") || e.getKey().equals("N+_m") || e.getKey().equals("TS+_m")
								|| e.getKey().equals("O+_m"))
						.mapToDouble(e -> e.getValue()).sum());
		...
		mapCodeSummary.put("SUM_MICO",
				mapCodeCount.entrySet().stream()
						.filter(e -> e.getKey().equals("ADP") || e.getKey().equals("AF") || e.getKey().equals("EC")
								|| e.getKey().equals("RCP") || e.getKey().equals("SU"))
						.mapToDouble(e -> e.getValue()).sum());

		// sum(ADW,CO,DI,RCW,WA)
		mapCodeSummary.put("SUM_MIIN",
				mapCodeCount.entrySet().stream()
						.filter(e -> e.getKey().equals("ADW") || e.getKey().equals("CO") || e.getKey().equals("DI")
								|| e.getKey().equals("RCW") || e.getKey().equals("WA"))
						.mapToDouble(e -> e.getValue()).sum());
		...
		mapCodeSummary.put("SUM_QUESTION",
				mapCodeCount.entrySet().stream()
						.filter(e -> e.getKey().equals("CQ-") || e.getKey().equals("CQ0") || e.getKey().equals("CQ+")
								|| e.getKey().equals("OQ-") || e.getKey().equals("OQ0") || e.getKey().equals("OQ+"))
						.mapToDouble(e -> e.getValue()).sum());
		...
		mapCodeSummary.put("SUM_REFLECTION",
				mapCodeCount.entrySet().stream()
						.filter(e -> e.getKey().equals("SR+") || e.getKey().equals("SR-") || e.getKey().equals("SR0")
								|| e.getKey().equals("SR+/-") || e.getKey().equals("CR+") || e.getKey().equals("CR-")
								|| e.getKey().equals("CR0") || e.getKey().equals("CR+/-"))
						.mapToDouble(e -> e.getValue()).sum());

		// sum(Rem+, rem+_m)
		mapCodeSummary.put("SUM_REM_POS",
				mapCodeCount.entrySet().stream().filter(e -> e.getKey().equals("Rem+") || e.getKey().equals("Rem+_m"))
						.mapToDouble(e -> e.getValue()).sum());
		...
		mapCodeSummary.put("SUM_FN_RM",
				mapCodeCount.entrySet().stream()
						.filter(e -> e.getKey().equals("FN") || e.getKey().equals("Rem+") || e.getKey().equals("Rem-")
								|| e.getKey().equals("Rem+_m") || e.getKey().equals("Rem-_m"))
						.mapToDouble(e -> e.getValue()).sum());

		// sum(SR+, CR+)
		mapCodeSummary.put("SUM_REF_CT",
				mapCodeCount.entrySet().stream().filter(e -> e.getKey().equals("SR+") || e.getKey().equals("CR+"))
						.mapToDouble(e -> e.getValue()).sum());
		...
		// client_utt = sum( C+ R+ D+ A+ N+ TS+ O+ C- R- D- A- N- TS- O- C+_m
		// R+_m D+_m A+_m N+_m TS+_m O+_m C-_m R-_m D-_m A-_m N-_m TS-_m O-_m
		// FN, Rem+, Rem-, Rem+_m, Rem-_m).
		mapCodeSummary.put("SUM_CLIENT_UTT",
				mapCodeCount.entrySet().stream()
						.filter(e -> e.getKey().equals("C+") || e.getKey().equals("R+") || e.getKey().equals("D+")
								|| e.getKey().equals("A+") || e.getKey().equals("N+") || e.getKey().equals("TS+")
								|| e.getKey().equals("O+") || e.getKey().equals("C-") || e.getKey().equals("R-")
								|| e.getKey().equals("D-") || e.getKey().equals("A-") || e.getKey().equals("N-")
								|| e.getKey().equals("TS-") || e.getKey().equals("O-") || e.getKey().equals("C+_m")
								|| e.getKey().equals("R+_m") || e.getKey().equals("D+_m") || e.getKey().equals("A+_m")
								|| e.getKey().equals("N+_m") || e.getKey().equals("TS+_m") || e.getKey().equals("O+_m")
								|| e.getKey().equals("C-_m") || e.getKey().equals("R-_m") || e.getKey().equals("D-_m")
								|| e.getKey().equals("A-_m") || e.getKey().equals("N-_m") || e.getKey().equals("TS-_m")
								|| e.getKey().equals("O-_m") || e.getKey().equals("") || e.getKey().equals("FN")
								|| e.getKey().equals("Rem+") || e.getKey().equals("Rem-") || e.getKey().equals("Rem+_m")
								|| e.getKey().equals("Rem-_m"))
						.mapToDouble(e -> e.getValue()).sum());

		// ther_utt = sum(ADP ADW AF CO DI EC FA FI GI CQ- CQ0 CQ+ OQ- OQ0 OQ+
		// RCP RCW SR+ SR- SR0 SR+/- CR+ CR- CR0 CR+/- RF ST SU WA, p)
		mapCodeSummary.put("SUM_THER_UTT",
				mapCodeCount.entrySet().stream()
						.filter(e -> e.getKey().equals("ADP") || e.getKey().equals("ADW") || e.getKey().equals("AF")
								|| e.getKey().equals("CO") || e.getKey().equals("DI") || e.getKey().equals("EC")
								|| e.getKey().equals("FA") || e.getKey().equals("FI") || e.getKey().equals("GI")
								|| e.getKey().equals("CQ-") || e.getKey().equals("CQ0") || e.getKey().equals("CQ+")
								|| e.getKey().equals("OQ-") || e.getKey().equals("OQ0") || e.getKey().equals("OQ+")
								|| e.getKey().equals("RCP") || e.getKey().equals("RCW") || e.getKey().equals("SR+")
								|| e.getKey().equals("SR-") || e.getKey().equals("SR0") || e.getKey().equals("SR+/-")
								|| e.getKey().equals("CR+") || e.getKey().equals("CR-") || e.getKey().equals("CR0")
								|| e.getKey().equals("CR+/-") || e.getKey().equals("RF") || e.getKey().equals("ST")
								|| e.getKey().equals("SU") || e.getKey().equals("WA") || e.getKey().equals("P"))
						.mapToDouble(e -> e.getValue()).sum());
		...
		mapCodeSummary.put("SUM_TOTAL_UTT",
				(mapCodeSummary.get("SUM_THER_UTT") + mapCodeSummary.get("SUM_CLIENT_UTT")));
		// Change = sum(change_etoh, change_drug)
		mapCodeSummary.put("SUM_CHANGE",
				(mapCodeSummary.get("SUM_CHANGE_ETOH") + mapCodeSummary.get("SUM_CHANGE_DRUG")));
		// Sustain = sum(sustain_etoh, sustain_drug).
		mapCodeSummary.put("SUM_SUSTAIN",
				(mapCodeSummary.get("SUM_SUSTAIN_ETOH") + mapCodeSummary.get("SUM_SUSTAIN_DRUG")));
		...
		setVal = ((mapCodeSummary.get("SUM_MICO") / (mapCodeSummary.get("SUM_MICO") + mapCodeSummary.get("SUM_MIIN")))
				* 100);
		mapCodeSummary.put("PCT_MIC", setVal.isNaN() || setVal.isInfinite() ? 0.0 : setVal);
		// pct_etoh = change_etoh/sum(change_etoh,sustain_etoh).
		setVal = ((mapCodeSummary.get("SUM_CHANGE_ETOH")
				/ (mapCodeSummary.get("SUM_CHANGE_ETOH") + mapCodeSummary.get("SUM_SUSTAIN_ETOH"))) * 100);
		...
		setVal = ((mapCodeSummary.get("SUM_CHANGE_DRUG")
				/ (mapCodeSummary.get("SUM_CHANGE_DRUG") + mapCodeSummary.get("SUM_SUSTAIN_DRUG"))) * 100);
		mapCodeSummary.put("PCT_DRUG", setVal.isNaN() || setVal.isInfinite() ? 0.0 : setVal);
		// pct = change/sum(change, sustain).
		setVal = ((mapCodeSummary.get("SUM_CHANGE")
				/ (mapCodeSummary.get("SUM_CHANGE") + mapCodeSummary.get("SUM_SUSTAIN"))) * 100);
	...
	public String getUtteranceAnnotationText(String utterance_id) throws SQLException {
		try (Connection connection = ds.getConnection();
				PreparedStatement ps = connection
						.prepareStatement("select annotation from utterances where utterance_id = ?")) {
	...
	public void annotateUtterance(String utterance_id, String annotationText, ArrayList<GlobalCode> globalsList)
			throws SQLException {
		try (Connection connection = ds.getConnection();
				PreparedStatement psU = connection
						.prepareStatement("update utterances set annotation = ? where utterance_id = ?");
				PreparedStatement psI = connection
						.prepareStatement("INSERT INTO utterances_ratings (utterance_id, rating_id) VALUES (?, ?)");
			...
			// there is probably a better design pattern for doing this where
			// when the instance gets updated the persistence is updated
			this.utteranceList.get(utterance_id).setAnnotation(annotationText);

			// System.out.println("--- annotateUtterance: Update annotation
			// string:"+utterance_id);
			...
			// System.out.println("--- annotateUtterance: Clear rating
			// links:"+utterance_id);
			...
			// System.out.println("--- annotateUtterance: Insert rating
			// links:"+utterance_id);
			psI.executeBatch();

			// System.out.println("--- annotateUtterance:
			// commit:"+utterance_id);
			// apply changes
			connection.commit();
		}
	...
	private void removeUtterance(String utterance_id) throws SQLException {
		...
		try (Connection connection = ds.getConnection();
				PreparedStatement ps1 = connection
						.prepareStatement("delete from utterances_ratings where utterance_id = ?");
	 ...
	 * @throws SQLException
	 */
	private void addUtterance(String utterance_id, int code_id, String time_marker, String annotation)
			throws SQLException {

		/*
		 * utterance accounting
		 * 
		 * System.out.println( String.format("--- Add utterance: %s - %s - %s",
		 * utterance_id, time_marker, annotation) );
		 */
	...
	private void setRatingResponseValue(int rating_id, int response_value) throws SQLException {
		...
		String sql = "select rating_id from ratings where rating_id = ?";
		boolean isEmpty;
		try (Connection connection = ds.getConnection(); PreparedStatement ps = connection.prepareStatement(sql)) {
			ps.setString(1, rating_id + "");
			ResultSet rs = ps.executeQuery();

			// check if there is a valid rating for the given id
			isEmpty = !rs.isBeforeFirst();
		}

		if (!isEmpty) {
			sql = "update ratings set response_value = ? where rating_id = ?";
			...
			sql = "insert into ratings (rating_id, response_value, rating_name) values (?,?,?)";

			try (Connection connection = ds.getConnection(); PreparedStatement ps = connection.prepareStatement(sql)) {
				ps.setInt(1, rating_id);
				ps.setInt(2, response_value);
				ps.setString(3, GlobalCode.codeWithId(rating_id).name);
				ps.executeUpdate();
			}
		}
	...
	/**
	 * Initialize datasource schema and default values Codes and their defaults
	 * initialized from UserConfig
	 * 
	 * @throws SQLException
	 */
	private void initDB() throws SQLException {
		...
		try (Connection connection = ds.getConnection(); Statement statement = connection.createStatement()) {
			...
			statement.executeUpdate("create table if not exists speakers ( "
					+ "speaker_id integer primary key not null, " + "speaker_name string not null unique" + ")");
			statement.executeUpdate("create table if not exists codes ( " + "code_id integer primary key not null, "
			...
			statement.executeUpdate(
					"create table if not exists utterances ( " + "utterance_id TEXT primary key not null unique, "
							+ "time_marker string not null, " + "code_id integer, " + "annotation string,"
							+ "  foreign key (code_id) references codes (code_id)" + ")");
			statement.executeUpdate("create table if not exists ratings ( " + "rating_id integer primary key not null, "
					+ "rating_name string not null unique," + "response_value integer not null" + ")");
			statement.executeUpdate("create table if not exists utterances_ratings ( " + "utterance_id TEXT,"
					+ "rating_id integer, " + "  foreign key (utterance_id) references utterances (utterance_id),"
					+ "  foreign key (rating_id) references ratings (rating_id)" + ")");
			statement.executeUpdate("create table if not exists attributes ( name, value )");
			// assumes data file does not exists
			statement.executeUpdate("insert into attributes ( name, value ) values ( '"
	...
	public class UtteranceList {
		...
		/*
		 * A sortedMap of utterances, sorted by id which should be a string
	...
	public class Ratings {
		...
		// notes field for ratings overall. Could just be session notes at this
		// point.
	...
	public class Export {
		...
		public void writeCodeList() throws IOException {
			...
			String outFilePath = this.destinationPath + File.separator + "exported_codelist_"
					+ sessionFile.getName().replace(".casaa", ".txt");
	...
	public static class Compatibility {
		 ...
		 * @throws IOException
		 */
		static public SessionData sessionDataFromPreviousFileFormat(File sessionFile, File globalsFile)
				throws IOException {
			...
			System.out.println("Globals List: " + globalsList);
		 ...
		 * @throws IOException
		 */
		public static List<Utterance> utteranceListFromPreviousFileFormat(File sessionFileTextFormat)
				throws IOException {
			...
			while (in.hasNextLine()) {
				...
				if (lineSize == 3) {
					...
					try {
						...
						// if lookup failed there is a possible disconnect
						// between codes in casaa file
						// and codes in user config file
						throw new IOException(String.format(
								"The code (%s) with value (%d) in file (%s) was not found in the current user configuration file.\n\nIf you uncode (%s) you will not be able to recode it with the current config file.",
								codeName, codeId, sessionFileTextFormat.getName(), codeName));
					}
				...
				/* read 7 to handle old data format */
					...
					if (!startTime.equals(Duration.ZERO)) {
						...
						// look up parsed code in user config codes loaded at
						// initDB
						try {
							item.setMiscCodeByValue(codeId);
						} catch (Exception e) {
							...
							throw new IOException(String
									.format("Code(%d) in casaa file not found in user configuration file", codeId));
						}
		 ...
		 * @throws IOException
		 */
		public static HashMap<String, Integer> globalRatingsListFromPreviousFileFormat(File globalsFileTextFormat)
				throws IOException {
